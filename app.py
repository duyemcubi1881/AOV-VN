...
@app.route('/aov/api/keys', methods=['GET'])
def get_all_keys():
    keys = Key.query.all()
    key_list = []
    for key in keys:
...
@app.route('/aov/api/createkey', methods=['POST'])
def create_key():
...
@app.route('/aov/api/deletekey', methods=['POST'])
def delete_key():
    data = request.get_json()
...
@app.route('/aov/api/redeem', methods=['POST'])
def redeem_key():
    data = request.get_json()
    key_str = data.get('key')
    hwid = data.get('hwid')
...
@app.route('/aov/api/checkkey', methods=['GET'])
def check_key():
...
@app.route('/aov/api/ban', methods=['POST'])
def ban_key():
    data = request.get_json()
    key_str = data.get('key')
    if not key_str:
...
@app.route('/aov/api/unban', methods=['POST'])
def unban_key():
    data = request.get_json()
    key_str = data.get('key')
    if not key_str:
